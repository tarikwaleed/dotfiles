...
complete -c git-grep -l recurse-submodules -d 'Recursively search in each submodule that has been initialized and checked ou…'
...
complete -c git-grep -s z -l null -d 'Output 0 instead of the character that normally follows a file name'
