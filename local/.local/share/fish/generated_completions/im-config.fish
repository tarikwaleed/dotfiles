...
complete -c im-config -s a -d 'List all; force to list all possible input methods in menu or STDOUT even if …'
...
complete -c im-config -s l -d 'List all available input method configuration settings to STDOUT'
complete -c im-config -s m -d 'List configuration values to STDOUT: . RS 12 . ie n \\{\\ \\h\'-04\'\\(bu\\h\'+03\'\\c '
complete -c im-config -s n -d '<input method name>  Set input method to <input method name>'
