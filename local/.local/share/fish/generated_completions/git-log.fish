...
complete -c git-log -l use-mailmap -d 'Use mailmap file to map author and committer names and email addresses to can…'
...
complete -c git-log -s L -s L -d 'Trace the evolution of the line range given by "<start>,<end>" (or the functi…'
...
complete -c git-log -l abbrev-commit -d 'Instead of showing the full 40-byte hexadecimal commit object name, show only…'
...
complete -c git-log -s c -d 'With this option, diff output for a merge commit shows the differences from e…'
complete -c git-log -l cc -d 'This flag implies the -c option and further compresses the patch output by om…'
complete -c git-log -l combined-all-paths -d 'This flag causes combined diffs (used for merge commits) to list the name of …'
complete -c git-log -s m -d 'This flag makes the merge commits show the full diff like regular commits; fo…'
complete -c git-log -s r -d 'Show recursive diffs'
complete -c git-log -s t -d 'Show the tree objects in the diff output.  This implies -r'
...
complete -c git-log -l unfold -d 'option was given'
