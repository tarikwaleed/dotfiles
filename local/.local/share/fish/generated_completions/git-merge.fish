...
complete -c git-merge -s S -l gpg-sign -d 'GPG-sign the resulting merge commit'
complete -c git-merge -l log -l no-log -d 'In addition to branch names, populate the log message with one-line descripti…'
complete -c git-merge -l signoff -l no-signoff -d 'Add Signed-off-by line by the committer at the end of the commit log message'
