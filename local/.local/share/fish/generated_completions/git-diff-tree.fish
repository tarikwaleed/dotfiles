...
complete -c git-diff-tree -l relative -d 'When run from a subdirectory of the project, it can be told to exclude change…'
...
complete -c git-diff-tree -s W -l function-context -d 'Show whole surrounding functions of changes'
...
complete -c git-diff-tree -l abbrev-commit -d 'Instead of showing the full 40-byte hexadecimal commit object name, show only…'
...
complete -c git-diff-tree -l unfold -d 'option was given'
