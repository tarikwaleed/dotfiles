...
complete -c youtube-dl -l load-info-json -d 'JSON file containing the video information (created with the \\[dq]--write-inf…'
...
complete -c youtube-dl -l rm-cache-dir -d 'Delete all filesystem cache files . SS Thumbnail images:'
...
complete -c youtube-dl -l video-password -d 'Video password (vimeo, smotri, youku) . SS Adobe Pass Options:'
...
complete -c youtube-dl -s x -l extract-audio -d 'Convert video files to audio-only files (requires ffmpeg or avconv and ffprob…'
...
complete -c youtube-dl -s k -l keep-video -d 'Keep the video file on disk after the post- processing; the video is erased b…'
...
complete -c youtube-dl -l exec -d 'Execute a command on the file after downloading, similar to find\\[aq]s -exec …'
complete -c youtube-dl -o exec -d 'Example: --exec [aq]adb push {} /sdcard/Music/ && rm {}[aq]'
