...
complete -c git-pull -l recurse-submodules -d 'This option controls if new commits of all populated submodules should be fet…'
...
complete -c git-pull -s S -l gpg-sign -d 'GPG-sign the resulting merge commit'
complete -c git-pull -l log -l no-log -d 'In addition to branch names, populate the log message with one-line descripti…'
complete -c git-pull -l signoff -l no-signoff -d 'Add Signed-off-by line by the committer at the end of the commit log message'
...
complete -c git-pull -l autostash -l no-autostash -d 'Before starting rebase, stash local modifications away (see git-stash(1)) if …'
...
complete -c git-pull -s u -l update-head-ok -d 'By default git fetch refuses to update the head which corresponds to the curr…'
