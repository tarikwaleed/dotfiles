...
complete -c git-rebase -l keep-empty -d 'Keep the commits that do not change anything from its parents in the result'
complete -c git-rebase -l allow-empty-message -d 'By default, rebasing commits with an empty message will fail'
...
complete -c git-rebase -s S -l gpg-sign -d 'GPG-sign commits'
...
complete -c git-rebase -l ignore-whitespace -l whitespace -d 'These flag are passed to the git apply program (see git-apply(1)) that applie…'
complete -c git-rebase -l committer-date-is-author-date -l ignore-date -d 'These flags are passed to git am to easily change the dates of the rebased co…'
