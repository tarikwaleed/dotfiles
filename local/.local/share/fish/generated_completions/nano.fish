...
complete -c nano -s O -l morespace -d 'Obsolete and ignored option, since the line below the title bar is included i…'
...
complete -c nano -s S -l smooth -d 'Obsolete and ignored option, since smooth scrolling has become the default'
complete -c nano -o Tnumber -l tabsize -d 'Set the size (width) of a tab to number columns'
complete -c nano -s U -l quickblank -d 'Do quick status-bar blanking: status-bar messages will disappear after 1 keys…'
...
complete -c nano -s t -l tempfile -d 'Save a changed buffer without prompting (when exiting with ^X)'
...
complete -c nano -s y -l afterends -d 'Make Ctrl+Right stop at word ends instead of beginnings'
complete -c nano -s z -l suspend -d 'Enable the suspend ability'
...
complete -c nano -s '$' -l softwrap -d 'Enable \'soft wrapping\''
