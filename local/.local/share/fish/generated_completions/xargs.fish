...
complete -c xargs -s 0 -d 'option, which prevents such problems'
...
complete -c xargs -s x -d and
complete -c xargs -s i -l replace -d 'This option is a synonym for'
complete -c xargs -s l -l max-lines -d 'Synonym for the'
...
complete -c xargs -l process-slot-var -d 'Set the environment variable name to a unique value in each running child pro…'
...
complete -c xargs -l version -d 'Print the version number of xargs and exit.  EXAMPLES'
