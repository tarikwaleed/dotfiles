...
complete -c dnsmasq -l rebind-localhost-ok -d 'Exempt 127. 0. 0. 0/8 from rebinding checks'
...
complete -c dnsmasq -l add-cpe-id -d 'Add an arbitrary identifying string to o DNS queries which are forwarded upst…'
...
complete -c dnsmasq -s C -l conf-file -d 'Specify a different configuration file'
complete -c dnsmasq -s 7 -l conf-dir -d 'Read all the files in the given directory as configuration files'
complete -c dnsmasq -l ra-parm -d '(A value of zero for the interval means the default value'
