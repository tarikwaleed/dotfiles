...
complete -c mpv -l video-rotate -d 'Rotate the video clockwise, in degrees.  Currently supports 90Â° steps only'
...
complete -c mpv -l ad-lavc-downmix -d 'Whether to request audio channel downmixing from the decoder (default: yes)'
...
complete -c mpv -l audio-display -d 'Setting this option to attachment (default) will display image attachments (e'
...
complete -c mpv -l ontop-level -d '(OS X only) Sets the level of an ontop window (default: window).  NDENT 7. 0'
complete -c mpv -l border -l no-border -d 'Play video with window border and decorations'
complete -c mpv -l fit-border -l no-fit-border -d '(Windows only) Fit the whole window with border and decorations on the screen'
complete -c mpv -l on-all-workspaces -d '(X11 only) Show the video window on all virtual desktops'
...
complete -c mpv -l hidpi-window-scale -l no-hidpi-window-scale -d '(OS X, X11, and Wayland only) Scale the window size according to the backing …'
complete -c mpv -l native-fs -l no-native-fs -d '(OS X only) Uses the native fullscreen mechanism of the OS (default: yes)'
...
complete -c mpv -l demuxer-seekable-cache -d 'This controls whether seeking can use the demuxer cache (default: auto)'
...
complete -c mpv -l no-input-default-bindings -d 'Disable mpv default (built-in) key bindings'
...
complete -c mpv -l input-file -d 'Deprecated.  Use --input-ipc-server. sp Read commands from the given file'
...
complete -c mpv -l input-media-keys -d '(OS X and Windows only) Enable/disable media keys support'
...
complete -c mpv -l osd-back-color -d 'See --osd-color.  Color used for OSD text background'
...
complete -c mpv -l osd-border-color -d 'See --osd-color.  Color used for the OSD font border. sp NOTE: NDENT 7'
...
complete -c mpv -l wayland-frame-wait-offset -d 'Control the amount of offset (in microseconds) to add to wayland\\(aqs frame w…'
...
complete -c mpv -l cocoa-force-dedicated-gpu -d 'Deactivates the automatic graphics switching and forces the dedicated GPU'
...
complete -c mpv -l opengl-restrict -d 'Restricts all OpenGL versions above a certain version'
...
complete -c mpv -l icc-contrast -d 'Specifies an upper limit on the target device\\(aqs contrast ratio'
...
complete -c mpv -l video-sync-adrop-size -d 'For the --video-sync=display-adrop mode'
...
complete -c mpv -o set -d 'T}	T{ Set a list of items (using the list separator, interprets escapes) T} _…'
...
complete -c mpv -l oss-mixer-device -d 'Sets the audio mixer device (default: /dev/mixer)'
complete -c mpv -l oss-mixer-channel -d 'Sets the audio mixer channel (default: pcm)'
...
complete -c mpv -l sdl-bufcnt -d 'Sets the number of extra audio buffers in mpv.  Usually needs not be changed'
...
complete -c mpv -l vo-vdpau-deint -d '(Deprecated.  See note about vdpaupp. )'
...
complete -c mpv -l vo-direct3d-prefer-stretchrect -d 'Use IDirect3DDevice9::StretchRect over other methods if possible'
complete -c mpv -l vo-direct3d-disable-stretchrect -d 'Never render the video using IDirect3DDevice9::StretchRect'
complete -c mpv -l vo-direct3d-disable-textures -d 'Never render the video using D3D texture rendering'
complete -c mpv -l vo-direct3d-disable-shaders -d 'Never use shaders when rendering video'
complete -c mpv -l vo-direct3d-only-8bit -d 'Never render YUV video with more than 8 bits per component'
...
complete -c mpv -l v
complete -c mpv -l enable-cplugins -d 'C plugins location'
